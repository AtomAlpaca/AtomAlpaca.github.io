...
下文中，我们用 \texttt{ls[x]} 表示节点 $x$ 的左儿子，\texttt{rs[x]} 表示节点 $x$ 的右儿子，\texttt{vl[x]} 表示节点 $x$ 的权值，\texttt{sz[x]} 表示节点 $x$ 及其子树中叶子节点的个数。
...
\includegraphics[width=0.80\textwidth]{https://oi-wiki.org/ds/images/wblt-1.svg}

我们首先找到了叶子节点 $5$，随后新建了一个非叶子节点 $D$，并将 $4$ 和 $5$ 连接到了 $D$ 上。

\includegraphics[width=0.80\textwidth]{https://oi-wiki.org/ds/images/wblt-2.svg}
...
\includegraphics[width=0.80\textwidth]{https://oi-wiki.org/ds/images/wblt-3.svg}

这是一棵十分不平衡的 WBLT，节点 $A$ 的右儿子显著地重于左儿子。我们先把右儿子及其两个儿子和左儿子都拆下来：

\includegraphics[width=0.80\textwidth]{https://oi-wiki.org/ds/images/wblt-4.svg}

然后，我们将 $1$ 和 $2$ 两个节点合并作为 $A$ 节点的左儿子，将 $C$ 作为 $A$ 的右儿子。由于 $B$ 节点原本并不是叶子节点，因此其并不存储原始信息，直接删除就好。

\includegraphics[width=0.80\textwidth]{https://oi-wiki.org/ds/images/wblt-5.svg}

旋转之后我们的树就变得十分平衡了。

但是上面的例子中，假设 $A$ 节点的左子树过于大，我们把它合并到 $A$ 的左子树上之后 $A$ 的左子树又会很大，这时 $A$ 依然可能不平衡。

\includegraphics[width=0.80\textwidth]{https://oi-wiki.org/ds/images/wblt-6.svg}
...
\includegraphics[width=0.80\textwidth]{https://oi-wiki.org/ds/images/wblt-7.svg}
...
我们发现 WBLT 的形态和线段树十分相似，因此查询排名可以使用类似线段树上二分的方式：如果左子树的最大值比大于等于待查值就往左儿子跳，否则就向右跳，同时答案加上左子树的 \texttt{size}。
...
以上，我们利用 WBLT 完成了平衡树基本的几大操作。下面是用 WBLT 实现的 \href{https://loj.ac/p/104}{普通平衡树模板}。
